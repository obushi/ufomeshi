...
class V2016MAY
  ...
  def to_date date
    Date.new(Date.today.year, date.split("/")[0].tr("０-９","0-9").to_i, date.split("/")[1].tr("０-９","0-9").to_i)
  end
  ...
  def extract_header
    CSV.read(@csv_path).to_a.select{ |e| e.join.count("/") > 5 }.first
  end
  ...
  def extract_dates
    @csv.headers.select{ |e| e&.tr("０-９","0-9") =~ /\d+\/\d+.*/ }
  end
end
